...
describe("H3 #519: Operator access (full — backwards-compatible)", () => {
  it("GET /value — operator CANNOT reveal plaintext (requires admin scope)", async () => {
    // P316: GET /secrets/value was elevated to requireScope("admin") to prevent
    // non-admin operators from revealing plaintext secret values over the REST API.
  ...
  it("GET /value — admin can reveal plaintext secret value", async () => {
    const app = makeApp(makeProvider(), { role: "admin" });
    const res = await app.request("/api/v1/secrets/value?ns=divisions/finance&key=API_KEY");
    // Note: makeProvider() mock returns "secret-value" for any get() call
...
describe("H3 #519: Division-scoped agent — read access", () => {
  it("GET /value — agent CANNOT reveal secrets (requires admin scope)", async () => {
    // P316: Secret reveal elevated to admin scope — agents must use CLI or admin-delegated mechanism.
  ...
  it("GET /value — agent CANNOT reveal own division secrets (requires admin scope)", async () => {
    const app = makeApp(makeProvider(), { role: "agent", division: "engineering" });
  ...
  it("GET /value — agent denied cross-namespace (IDOR blocked, scope error code)", async () => {
    // Scope check fires before RBAC check, so error is AUTH-003 (scope), not SEC-403 (RBAC).
    const app = makeApp(makeProvider(), { role: "agent", division: "engineering" });
...
describe("H3 #519: Audit logging", () => {
  it("allowed reveal is recorded in audit log (admin role required)", async () => {
    // P316: GET /secrets/value now requires admin scope — use admin for this test.
    const app = makeApp(makeProvider(), { role: "admin", division: "engineering" });
  ...
  it("denied listing is recorded in audit log with 'denied' outcome", async () => {
    // GET /secrets/keys still uses requireScope("readonly") — use it to test RBAC denial auditing.
    // An agent trying to list keys from another division's namespace is denied and audited.
    const app = makeApp(makeProvider(), { role: "agent", agentId: "agent-007", division: "engineering" });
    await app.request("/api/v1/secrets/keys?ns=divisions%2Ffinance");
    const log = getSecretAuditLog();
    const entry = log.find((e) => e.op === "list" && e.outcome === "denied");
    expect(entry).toBeDefined();
