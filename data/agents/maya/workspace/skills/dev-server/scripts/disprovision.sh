#!/usr/bin/env bash
# disprovision.sh — Deprovision a Hetzner dev server by name or ID.
# Enforces minimum 15-minute lifetime from creation.
#
# Usage: disprovision.sh <server-name-or-id> [--force]
set -euo pipefail
...
TARGET="${1:?Usage: disprovision.sh <server-name-or-id> [--force]}"
FORCE="${2:-}"
