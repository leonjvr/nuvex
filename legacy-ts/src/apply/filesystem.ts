...
export function planFilesystem(config: ParsedConfig): FilesystemOp[] {
  ...
  // governance/ directories — always created regardless of mode
  ops.push({ type: "mkdir", path: "/governance", overwrite: false });

  // governance/orchestrator.yaml — generated on first apply (overwrite:false preserves customizations)
  // DUAL PATH: init.ts also writes this file via writeIfAbsent; keep content in sync with
  // src/defaults/orchestrator.yaml which is the canonical template reference.
  ops.push({
    type:      "write",
    path:      "/governance/orchestrator.yaml",
    content:   ORCHESTRATOR_YAML_TEMPLATE,
    overwrite: false,
  });
...
const ORCHESTRATOR_YAML_TEMPLATE = `# SIDJUA Orchestrator Configuration
# Auto-generated by sidjua apply — customize as needed.
#
# mode: single | distributed
#   single      — one orchestrator process (default, suitable for most deployments)
#   distributed — reserved for future cluster mode
#
# poll_interval: how often (ms) the orchestrator polls for new tasks (default: 500)
# max_concurrent: maximum simultaneously RUNNING tasks across all agents (default: 10)
# task_timeout: maximum task runtime in seconds before forced cancellation (default: 300)
# api_port: HTTP port for the REST API and GUI (default: 3000)
